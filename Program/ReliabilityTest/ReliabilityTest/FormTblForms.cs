...
{
    public partial class FormTblForms : Form
    {
        public FormTblForms()
        {
        ...
        private void FormTblForms_Load(object sender, EventArgs e)
        {
        ...
        private void SaveButtonClick(object sender, EventArgs e)
        {
            try
            {
                DataSetForms changes = (DataSetForms)dataSetForms.GetChanges();
                if (changes == null)
                    return;
                // check for errors
                DataTable dt = changes.tblForms.GetChanges();
                ...
                int numRows = tblFormsTableAdapter.Update(changes);
                MessageBox.Show("Updated " + numRows + " rows", "Success");
                dataSetForms.AcceptChanges();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error: " + ex.Message, "Erros",
                MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataSetForms.RejectChanges();
            }
        }
    }
}
